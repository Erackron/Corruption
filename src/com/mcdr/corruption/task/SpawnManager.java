...
public abstract class SpawnManager {
    ...
    public static void registerSpawner(Spawner spawner) {
        ...
        if (spawner.shouldStart()) {
            ...
            try {
                ...
                task.runTaskTimerAsynchronously(Corruption.in, spawner.getSpawnInterval(), spawner.getSpawnInterval());
            }
    ...
    public static boolean start(int id) {
        if (inactive.containsKey(id) && inactive.get(id).getSpawner().shouldStart()) {
            ...
            task.runTaskTimerAsynchronously(Corruption.in, task.getSpawner().getSpawnInterval(), task.getSpawner().getSpawnInterval());
            return true;
        }
        return false;
    }
    ...
    public static void startAll() {
        ...
        while (it.hasNext()) {
            ...
            if (cur.getValue().getSpawner().shouldStart()) {
                ...
                try {
                    cur.getValue().runTaskTimerAsynchronously(Corruption.in, spawnTime, spawnTime);
                } catch (IllegalStateException ignored) {
