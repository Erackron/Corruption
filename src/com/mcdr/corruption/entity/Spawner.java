...
public class Spawner {
    ...
    public boolean isInArea(Location loc) {
        return Utility.isNear(spawnpoint, loc, 0, radius);
    }
    ...
    public synchronized void spawn() {
        ...
        if (spawned.size() < spawnCap) {
            int random = Utility.Random(0, maxSpawnChance), curChance = 0;
            for (Entry<BossData, Integer> entry : spawnable.entrySet()) {
                curChance += entry.getValue();
                if (curChance > random) {
                    BossData bossData = entry.getKey();
                    int[] dimensions = Utility.getDimensions(bossData);
                    List<Block> validBlocks = findValidBlocks(dimensions[0], dimensions[1]);
                    if (validBlocks != null) {
                        Boss boss = CorEntityManager.spawnBossEntity(validBlocks.get(Utility.Random(0, validBlocks.size() - 1)).getLocation(), bossData.getEntityType(), bossData, this);
                        if (boss != null) {
                            spawned.add(boss);
                        }
                    }
    ...
    public synchronized Map<String, Object> getInfo() {
        ...
        info.put("spawninterval", Utility.round(spawnInterval / 20, 0) + "s");
