		...
		for (String rollString : rollStrings) {
            ...
            for(String drop:drops){
                if(!rollSection.isConfigurationSection(drop)){
                    CorLogger.w("'"+rollSection.getCurrentPath()+"."+drop+"' in the boss config file is invalid");
                    continue;
                }
                ...
                if(item==null){
                    CorLogger.w("'"+rollSection.getCurrentPath()+"."+drop+".Item' in the boss config file is invalid");
                    continue;
                }

                if(probability<0||probability>100){
                    CorLogger.w("'"+rollSection.getCurrentPath()+"."+drop+".Probability' in the boss config file is invalid");
                    continue;
                }

                if(minQuantity<0){
                    CorLogger.w("'"+rollSection.getCurrentPath()+"."+drop+".MinQuantity' in the boss config file is invalid");
                    continue;
                }
                if(maxQuantity<0){
                    CorLogger.w("'"+rollSection.getCurrentPath()+"."+drop+".MaxQuantity' in the boss config file is invalid");
                    continue;
                }
