...
public class Boss extends CorEntity implements CommandSender {
    ...
    public void updateCustomName(boolean force) {
        ...
        if (livingEntity.isCustomNameVisible() || force)
            livingEntity.setCustomName(MessageUtil.parseMessage(GlobalConfig.MessageParam.CUSTOMBOSSNAME.getMessage(), this));
    ...
    private void AddAbilities() {
        ...
        for (Ability ability : WorldConfig.getWorldData(livingEntity.getWorld()).getAbilities()) {
            if (MathUtil.random.nextInt(100) < ability.getAssignationChance())
    ...
    public void ActivateOnDeathAbilities(LivingEntity targetEntity, Location lastLoc) {
        for (Entry<Ability, Boolean> entry : abilities.entrySet()) {
            ...
            if (MathUtil.isNear(livingEntity.getLocation(), lastLoc, ability.getMinRange(), ability.getMaxRange())
    ...
    public void ActivateAbilities(LivingEntity livingEntity, ActivationCondition activationCondition, EntityDamageEvent event) {
        ...
        for (Entry<Ability, Boolean> entry : abilities.entrySet()) {
            ...
            if (MathUtil.isNear(livingEntity.getLocation(), getLivingEntity().getLocation(), ability.getMinRange(), ability.getMaxRange())
    ...
    public String getName() {
        return MessageUtil.parseMessage("{BOSSNAME}", getRawName());
    }
    ...
    protected void setSpawner(Spawner spawner) {
        ...
        livingEntity.setMetadata("spawner", new FixedMetadataValue(Corruption.getInstance(), spawner.getId()));
    }
    ...
    public void sendMessage(String message) {
        CorLogger.info(getName() + ": " + message);
    }
