...
public class ItemConfig extends BaseConfig {
    ...
    public static void Load() {
        ...
        if (!file.exists()) {
            copyResource(file, "config/items.yml");
        }
    ...
    public static void loadItems(YamlConfiguration yamlConfiguration) {
        ...
        for (String itemName : itemNames) {
            ...
            if (configurationSection == null) {
                CorLogger.warning("'" + itemName + "' in items config file is invalid.");
                continue;
            }
    ...
    public static CorItem processItemSection(ConfigurationSection configurationSection) {
        ...
        if (id < 0) {
            CorLogger.warning("'" + configurationSection.getCurrentPath() + ".Id' in the item config file is invalid");
            return null;
        }
        if (data < 0) {
            CorLogger.warning("'" + configurationSection.getCurrentPath() + ".Data' in the item config file is invalid");
            return null;
        }
        if (durability < 0) {
            CorLogger.warning("'" + configurationSection.getCurrentPath() + ".Durability' in the item config file is invalid");
            return null;
        }
        ...
        if (configurationSection.isConfigurationSection("Enchantments")) {
            ...
            for (String enchantment : enchantmentSection.getKeys(false)) {
                ...
                if (ench == null || enchName == null) {
                    CorLogger.warning("'" + enchantmentSection.getCurrentPath() + "." + enchantment + ".Enchantment' in the equipment config file is invalid.");
                    continue;
                }
                if (chance <= 0) {
                    CorLogger.warning("'" + enchantmentSection.getCurrentPath() + "." + enchantment + ".Probability' in the equipment config file is invalid.");
                    continue;
                }
                if (lvl <= 0) {
                    CorLogger.warning("'" + enchantmentSection.getCurrentPath() + "." + enchantment + ".Level' in the equipment config file is invalid.");
                    continue;
                }
