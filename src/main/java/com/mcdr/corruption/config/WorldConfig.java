...
public abstract class WorldConfig extends BaseConfig {
    ...
    public static void Load(World world) {
        ...
        if (!file.exists())
            copyResource(file, "config/world.yml");
    ...
    private static void LoadBosses(WorldData worldData, List<String> bossNames, String worldName) {
        ...
        for (String bossName : bossNames) {
            if (!bossesData.containsKey(bossName)) {
                CorLogger.warning("'" + bossName + "' in '" + worldName + "' config file isn't a valid boss.");
                continue;
            }
    ...
    private static void LoadAbilities(WorldData worldData, List<String> abilityNames, String worldName) {
        ...
        for (String abilityName : abilityNames) {
            if (!abilities.containsKey(abilityName)) {
                CorLogger.warning("'" + abilityName + "' in '" + worldName + " config file isn't a valid ability.");
                continue;
            }
    ...
    private static void LoadSpawners(ConfigurationSection spawners, String worldName) {
        ...
        for (String spawnerName : spawners.getKeys(false)) {
            ...
            if (id == 0) {
                CorLogger.warning("'Spawners." + spawnerName + ".id' in '" + worldName + " config file is invalid.");
                continue;
            }
            ...
            if (radius <= 0 || radius > 30) {
                CorLogger.warning("'Spawners." + spawnerName + ".Radius' in '" + worldName + " config file should be between 1 and 30.");
                continue;
            }
            int spawnInterval = spawners.getInt(spawnerName + ".SpawnInterval");
            if (spawnInterval <= 0) {
                CorLogger.warning("'Spawners." + spawnerName + ".SpawnInterval' in '" + worldName + " config file should be > 0.");
                continue;
            }
            int spawnCap = spawners.getInt(spawnerName + ".SpawnCap");
            if (spawnInterval <= 0) {
                CorLogger.warning("'Spawners." + spawnerName + ".SpawnCap' in '" + worldName + " config file should be > 0.");
                continue;
            }

            ConfigurationSection bosses = spawners.getConfigurationSection(spawnerName + ".Bosses");
            if (bosses == null) {
                CorLogger.info("'Spawners." + spawnerName + "' in '" + worldName + "' doesn't have any Bosses set to spawn. (Set value of Bosses to {} for this spawner to suppress this message)");
                continue;
            }
            ...
            for (String boss : bosses.getKeys(false)) {
                if (!bossesData.containsKey(boss)) {
                    CorLogger.warning("'" + boss + "' in '" + worldName + "' config file isn't a valid boss.");
                    continue;
                }
    ...
    private static void LoadLoots(WorldData worldData, ConfigurationSection lootSection, String worldName) {
        if (lootSection == null) {
            CorLogger.warning("'Loot' in '" + worldName + "' config file is invalid.");
            return;
        }
        ...
        for (String rollString : rollStrings) {
            ...
            if (rollSection == null) {
                CorLogger.warning("'Loot." + rollString + "' in '" + worldName + "' config file is invalid.");
                continue;
            }
            ...
            for (String drop : drops) {
                if (!rollSection.isConfigurationSection(drop)) {
                    CorLogger.warning("'" + rollSection.getCurrentPath() + "." + drop + "' in the " + worldName + " config file is invalid");
                    continue;
                }
                ...
                if (probability < 0 || probability > 100) {
                    CorLogger.warning("'" + rollSection.getCurrentPath() + "." + drop + ".Probability' in the " + worldName + " config file is invalid");
                    continue;
                }

                if (minQuantity < 0) {
                    CorLogger.warning("'" + rollSection.getCurrentPath() + "." + drop + ".MinQuantity' in the " + worldName + " config file is invalid");
                    continue;
                }
                if (maxQuantity < 0) {
                    CorLogger.warning("'" + rollSection.getCurrentPath() + "." + drop + ".MaxQuantity' in the " + worldName + " config file is invalid");
                    continue;
                }
