...
public abstract class AbilityConfig extends BaseConfig {
    ...
    public static void Load() {
        ...
        if (!file.exists())
            copyResource(file, "config/abilities.yml");
                ...
                case SUMMON:
                    ...
                    if (abilityEntries.containsKey(entryKey)) {
                        ...
                        for (String monsterNode : monsterTypeSection.getKeys(false)) {
                            ...
                            if (type == null) {
                                CorLogger.warning("'" + monsterNode + "' in '" + abilityName + "' in abilities.yml is not a valid EntityType");
                                continue;
                            }
                            int chance = 1;
                    ...
                    for (String activationCondition : activationConditions) {
                        ...
                        if (condition != null) {
                            if (condition == ActivationCondition.ONDEATH && !ability.getAbilityType().isOnDeathAllowed()) {
                                CorLogger.info("'" + abilityName + "." + entryKey + "." + activationCondition + "' isn't possible/allowed for this ability.");
                                continue;
                            }
