    ...
    public static void loadEquipmentSets(YamlConfiguration yamlConfig) {
        ...
        for (String equipmentSetName : equipmentSetNames) {
            ...
            if (configurationSection == null) {
                CorLogger.warning("'" + equipmentSetName + "' in equipment config file is invalid.");
                continue;
            }
    ...
    public static Map<CorItem, Integer[]> processEquipmentSection(YamlConfiguration yamlConfig, ConfigurationSection configurationSection, String equipmentType) {
        ...
        for (String equip : equipment) {
            ...
            if (yamlConfig.getConfigurationSection(eSPath) == null) {
                CorLogger.warning("'" + eSPath + "' in the equipment config file is invalid.");
                continue;
            }
            CorItem item = ItemConfig.items.get(equip);
            if (item == null) {
                CorLogger.warning("'" + eSPath + "' in the equipment config file does not exist.");
                continue;
            }
            ...
            if (prob <= 0) {
                CorLogger.warning("'" + eSPath + ".Probability' in the equipment config file is invalid.");
                continue;
            }
            if (dropProb < 0 || dropProb > 100) {
                CorLogger.warning("'" + eSPath + ".DropProbability' in the equipment config file is invalid.");
                continue;
            }
