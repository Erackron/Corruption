...
public abstract class IgnoreCommand extends BaseCommand {
    public static void process() {
        ...
        if (!sender.hasPermission("cor.ignore.immediate") && delay != 0) {
            ...
            if (ignoreTaskId != 0) {
                ...
                sender.sendMessage(ChatColor.GOLD + "[" + Corruption.getPluginName() + "] " + ChatColor.WHITE + "Ignore: " + ChatColor.GRAY + "Canceled");
            } else {
                corPlayer.setIgnoreTaskId(Corruption.scheduler.scheduleSyncDelayedTask(Corruption.getInstance(), new IgnoreCommandTask(corPlayer), delay * 20));
                sender.sendMessage(ChatColor.GOLD + "[" + Corruption.getPluginName() + "] " + ChatColor.WHITE + "Ignore: " + ChatColor.GRAY + "Applied in " + ChatColor.GREEN + delay + ChatColor.GRAY + " second(s)");
            }
        } else
            Apply(corPlayer);
    }
    ...
    public static void Apply(CorPlayer labPlayer) {
        ...
        sender.sendMessage(ChatColor.GOLD + "[" + Corruption.getPluginName() + "] " + ChatColor.WHITE + "Ignore: " + ChatColor.GREEN + !ignore);
    }
}
