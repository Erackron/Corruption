...
public class CorConfigUpdater {
    ...
    public CorConfigUpdater() {
        latestVersion = Corruption.getInstance().getDescription().getVersion();
    }
    ...
    private void updateAbilityConfig() {
        ...
        if (!VersionUtil.isOlderVersion(configVersion, latestVersion))
            return;

        CorLogger.info("Creating backup of abilities.yml!");
        File configFile = getFile("abilities.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "abilities.yml");

        new File(backupFile.getParent()).mkdirs();

        try {
            FileUtil.fileToFile(configFile, backupFile);
        } catch (Exception e) {
            e.printStackTrace();
        }

        CorLogger.info("Ability config backup created, updating abilities.yml");


        if (VersionUtil.isOlderVersion(configVersion, "2.0")) {
            ...
            for (String node : ability.getKeys(false)) {
                ...
                if (!ability.isSet(node + ".Probability")) {
                    CorLogger.warning("Missing values for ability '" + node + ".Probability' in abilities.yml");
                    continue;
                }
                ...
                if (ability.getString(node + ".Type").equals("Bomb")) {
                    if (!ability.isSet(node + ".Radius")) {
                        CorLogger.warning("Missing values for ability '" + node + "' in abilities.yml");
                        continue;
                    }
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.1")) {
            CorLogger.info("Updating abilities.yml");
            ...
            for (String node : ability.getKeys(false)) {
                ...
                if (!ability.isSet(node + ".Type")) {
                    CorLogger.info("Missing type in abilities.yml");
                    continue;
                }
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.4.1")) { //TODO
            CorLogger.info("Updating abilities.yml");
            for (String node : ability.getKeys(false)) {
                if (ability.isString(node + ".Type") && ability.getString(node + ".Type").equalsIgnoreCase("Summon")) {
                    ...
                    if (ability.isString(node + ".MonsterType")) {
                        ...
                        CorLogger.warning("'MonsterType' in '" + node + "' in abilities.yml is not a valid EntityType");
                        continue;
                    }
        ...
        CorLogger.info("Ability config updated");
    ...
    private void updateGlobalConfig() {
        ...
        CorLogger.info("Creating backup of config.yml");
        File configFile = getFile("config.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "config.yml");
        ...
        CorLogger.info("Global config backup created, updating config.yml");
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.1.2")) {
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.2")) {
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.2.1")) {
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.4")) {
        ...
        CorLogger.info("Global config updated");
    }
    ...
    private void updateBossConfig() {
        ...
        CorLogger.info("Creating backup of bosses.yml");

        File configFile = getFile("bosses.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "bosses.yml");
        ...
        CorLogger.info("Bosses config backup created, updating bosses.yml");
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.0")) {
            ...
            for (String node : bosses.getKeys(false)) {
                ...
                if (spawnString == null) {
                    CorLogger.warning("'" + node + "' in bosses config file is missing.");
                    return;
                }
                ...
                if (spawnValues.length < 2) {
                    CorLogger.warning("Missing values for '" + node + ".Spawn' in bosses config file");
                    return;
                ...
                if (!bosses.isSet(node + ".Stats")) {
                    CorLogger.warning("'" + node + ".Stats' in bosses config file is missing.");
                    return;
                }
                ...
                if (statsValues.length < 3) {
                    CorLogger.warning("Missing values for '" + node + ".Stats' in bosses config file");
                    return;
                }
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.1.1")) {
            for (String node : bosses.getKeys(false)) {
                if (node.equalsIgnoreCase("ConfigVersion"))
                    continue;
        ...
        if (VersionUtil.isOlderVersion(configVersion, "2.4")) {
            ...
            for (String node : bosses.getKeys(false)) {
                ...
                if (!bosses.isSet(node + ".Loot")) {
                    CorLogger.warning("'" + node + ".Loot' in bosses config file is missing");
                    continue;
                }
                ...
                for (String rollNode : lootSection.getKeys(false)) {
                    ...
                    for (String itemNode : rollSection.getKeys(false)) {
                        ...
                        if (itemValues.length < 4) {
                            CorLogger.warning("Missing values for '" + node + ".Loot." + rollNode + "." + itemNode + "' in bosses config file.");
                            continue;
                        }
        ...
        CorLogger.info("Boss config updated");
    }
    ...
    private void updateEquipmentConfig() {
        ...
        CorLogger.info("Creating backup of equipment.yml");
        File configFile = getFile("equipment.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "equipment.yml");
        ...
        CorLogger.info("Equipment config backup created, updating equipment.yml");
        ...
        CorLogger.info("Equipment config updated");
    }
    ...
    private void updateMagicSpellsConfig() {
        ...
        CorLogger.info("Creating backup of magicspells.yml");
        File configFile = getFile("magicspells.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "magicspells.yml");
        ...
        CorLogger.info("MagicSpells config backup created, updating magicspells.yml");
        ...
        CorLogger.info("MagicSpells config updated");
    }
    ...
    private void updateWorldConfig() {
        ...
        for (File worldFile : worldFolder.listFiles()) {
            ...
            CorLogger.info("Creating backup of " + worldFile.getName());
            File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "Worlds" + File.separator + worldFile.getName());

            new File(backupFile.getParent()).mkdirs();

            try {
                FileUtil.fileToFile(worldFile, backupFile);
            } catch (Exception e) {
                e.printStackTrace();
            }

            CorLogger.info("World " + worldFile.getName().replace(".yml", "") + " config backup created, updating " + worldFile.getName());
                ...
                if (lootSection != null) {
                    for (String rollNode : lootSection.getKeys(false)) {
                        ...
                        for (String itemNode : rollSection.getKeys(false)) {
                            ...
                            if (itemValues.length < 4) {
                                CorLogger.warning("Missing values for 'Loot." + rollNode + "." + itemNode + "' in " + worldFile.getName() + " config file.");
                                continue;
                            }
            ...
            CorLogger.info("" + worldFile.getName() + " config updated");
        }
    }
    ...
    private void updateItemConfig() {
        ...
        CorLogger.info("Creating backup uf items.yml");
        File configFile = getFile("items.yml");
        File backupFile = new File(Corruption.getInstance().getDataFolder().getPath(), "old_files" + File.separator + "v" + configVersion + File.separator + "items.yml");
        ...
        CorLogger.info("Items config backup created, updating items.yml");

        updateConfigVersion(itemConfig);
        save(itemConfig, "items.yml");
        CorLogger.info("Item config updated");
    }
    ...
    private File getFile(String name) {
        return new File(Corruption.getInstance().getDataFolder().getPath() + File.separatorChar + name);
    }
