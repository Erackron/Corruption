...
public abstract class CorruptionAPI {
    ...
    public static String getRandomBossName(EntityType entityType) {
        ...
        if (bossNames == null)
            return null;
        return bossNames.get(MathUtil.Random(0, bossNames.size() - 1));
    }
    ...
    /**
     * Use {@link com.mcdr.corruption.CorruptionAPI#isUsingAbsoluteHealth(org.bukkit.entity.Entity e)} to check if this value is a multiplier or an absolute value
     *
     ...
     * @see com.mcdr.corruption.CorruptionAPI#isUsingAbsoluteHealth(org.bukkit.entity.Entity e)
     */
    ...
    public static String getName(Entity e) {
        if (isBoss(e))
            return MessageUtil.parseMessage("{BOSSNAME}", getBoss(e));
        return "";
    }
    ...
    public static boolean isNear(Location first, Location second, int minDistance, int maxDistance) {
        return MathUtil.isNear(first, second, minDistance, maxDistance);
    }
    ...
    public static String getMetaTag(Entity e, String metaKey) {
        ...
        List<MetadataValue> list = e.getMetadata(metaKey);

        for (MetadataValue metaV : list) {
            if (!(metaV instanceof FixedMetadataValue))
                continue;
            else if (!metaV.getOwningPlugin().equals(Corruption.getInstance()))
                continue;
            return metaV.value() + "";
        }
