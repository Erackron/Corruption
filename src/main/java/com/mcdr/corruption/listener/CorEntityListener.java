...
public class CorEntityListener implements Listener {
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        ...
        double chance = MathUtil.random.nextInt(100), curChance = 0;
    ...
    private void addBoss(LivingEntity livingEntity, BossData bossData) {
        ...
        if (CorLogger.debugEnabled()) {
            CorLogger.debug("|BossData selection finished:\t\t\t|");
            CorLogger.debug("|BossName:\t\t" + bossData.getName());
            CorLogger.debug("|Abilities assigned:\t\t\t\t|");
            for (String aName : boss.abilityList())
                CorLogger.debug("|" + aName);
        }
    }
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (boss != null) {
                    ...
                    if (player != null) {
                        ...
                        if (enchants.containsKey(Enchantment.FIRE_ASPECT))
                            if (boss.getBossData().getImmunities().contains(BossImmunity.ENCHANT_FIRETICK_IMMUNE)) {
                                ...
                                Bukkit.getScheduler().scheduleSyncDelayedTask(Corruption.getInstance(), new GetFireEnchantTicks(boss), 0);
                    }
            ...
            Corruption.scheduler.scheduleSyncDelayedTask(Corruption.getInstance(), new ProcessEntityDamage(damager, boss, boss.getHealth(), livingEntity.getLocation()), 2L);
        }
    }
